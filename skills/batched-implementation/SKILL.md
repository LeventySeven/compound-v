...
4. On `APPROVED`, **commit the batch**. Mark each task completed in TodoWrite the moment *its* verification passes — don't hold todo updates until the commit; the batch boundary governs the **commit**, not the todo. Git is the state machine, and the thing it has to record is which point was *rechecked* — not merely which point was written. Expect the implementer to have committed as it went: the plan's task template carries a `Commit:` step per task (**compound-v:writing-plans**), and that test-first rhythm is right and worth keeping. Those intermediate commits are work, not verdicts — none of them has been through recheck. So on `APPROVED` the batch's **final** commit is the one that carries the trailer below: make it if there is uncommitted work, and amend it onto the last commit of the range if the implementer already committed everything, rather than manufacturing an empty commit to hold a trailer. That trailer is what promotes a commit from written to verified, so a later batch that breaks the build rolls back to the last known-good point without losing earlier work. When the run carries a state file, that trailered commit is also where **Done** moves (**compound-v:handoff**); the implementer's intermediate commits are exempt from its never-commit-without-moving-the-state-file rule, because they are not yet verified and the state file records what is.
   ...
   **Put the batch's identity and its verdict in the commit message.** `git log` is the resume oracle above and the durable record this kit already relies on, and neither works if the message is free prose — reconciling a diff back to plan tasks by eye is exactly the re-derivation the resume protocol exists to avoid. Close the batch commit with two trailer lines:
   ...
   The first line makes resume exact. The second is the one place the review's **negative result** survives: `compound-v:recheck` requires a verdict carrying what it *checked and found clean*, and that is precisely the finding a diff cannot represent — a check that found nothing and a check that never ran leave identical traces in git. One line, in the commit, and both become greppable. It also preserves one side of a comparison this kit asks you to make: `compound-v:writing-plans` says to delete its fresh-context pass if that pass only re-finds what recheck would have caught, and the trailer is what keeps recheck's half of that on the record — the plan pass's own findings are still nowhere in git, so the comparison stays partial until they are too.
