...
7. **Sub-agents (context isolation)** — split work into fresh, isolated context windows. The real reason is **context control**, not role-play "my PM / my QA": a sub-agent is a *context firewall* — it does the token-heavy reading/searching and returns only a distilled digest, so the raw material never touches the parent (the mechanism, and when this pays off, lives in compound-v:context-engineering). It doubles as a **failure firewall**: run an iteration-prone subtask inside it until it succeeds and return only the success plus a brief note of what *didn't* work, so the parent learns enough to steer away from the dead ends without inheriting the noise of every failed attempt. But it starts clean-slate (sees none of your history) and pays a latency tax to re-gather context, plus a "telephone" risk on what it returns — so don't reach for it when the task needs tight back-and-forth or shares a lot of context with the main thread.
...
- **Fix the tool, not the prompt around it.** When the agent keeps misusing a tool, treat the tool as an eval target: run it many times, watch where the model trips, then rewrite the interface and description *in the agent's own voice* — don't patch the calling prompt around a bad tool. A dedicated tool-testing pass that did exactly this cut downstream task-completion time ~40%; the model's ergonomics aren't a human's, so let its own failures redesign the tool.
- **Make the tool dumb and deterministic, not agentic.** A tool that is itself an LLM or sub-agent is hard to reason about and chains two non-deterministic systems, compounding failure. Prefer a plain deterministic action (a literal web search, a direct lookup) over "ask a sub-agent to figure it out" — push the intelligence into the *calling* agent and keep the tool a predictable primitive. (Doing the opposite backfires: wrapping a sub-LLM inside an output tool to fix tone increases latency *and* reduces output quality — the sub-tool lacks context and leaks internal steps.)
...
- **Gate irreversible actions behind a human (HITL).** Deleting data, sending a message, deploying, spending money — anything you can't undo gets an approval checkpoint before it runs, not a post-hoc apology. Reversible actions can run free; the gate is for the one-way doors. Make that gate **checkpoint-backed**: the approval may land hours later or in a different process, so persist state and resume rather than blocking a live one, and keep the HITL tool idempotent up to the request.
- **Durability is a separate axis from this ladder.** Climb the ladder for *capability*; add durability independently, when the work must survive a crash. Any rung can run ephemeral-and-simple by default and be promoted to durable with a flag — never a rewrite, never a forced graph (compound-v:ai-system-reliability owns the durable-state seams).
