...
When you *do* pack a large retrieved or assembled payload into the prompt, wrap it in **XML-style tags, not JSON** — tags read like the prose the model trained on and act as navigation anchors in long context, while JSON's structural characters (`{}":,`) are pure token overhead (Anthropic, prompt-engineering guidance on XML tags). On a large payload the token savings from the format alone are substantial.

The **tool *set* is always-loaded context too** — definitions sit in the prefix every turn, so a sprawling or ambiguous tool list taxes the attention budget before the model reads a single request: if a human engineer can't definitively say which tool to use in a given situation, an agent can't either. Carry a minimal, non-overlapping set; when you genuinely need many, expose them progressively (a `search_tools` call, or tools-as-files read on demand) rather than dumping every definition up front.
