...
**The ledger is the scoreboard, not the engine.** It withholds the verdict; it does not make an agent that has already stopped talking start again. Something else has to re-invoke you — a harness that keeps going until the task is resolved, a stop-hook that refuses the exit while rows are open, a human reading the delta. Name which one you are relying on at the start of the run, because a gate nobody returns to is just a record of where you stopped. Four rules hold it up — **references/completion-ledger.md** carries the row shape and the operational detail; read it when you open a ledger and again at the gate.
