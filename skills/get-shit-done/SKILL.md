...
You reach 100% by passing a row or by cutting it out loud with a name on it, never by forgetting it. That is what makes *"we're at about 90%"* unsayable: either the ledger names the other 10% and who dropped each one, or the run is not finished.

**The ledger needs an engine, or it is only a record of where you stopped.** Withholding the verdict does nothing to an agent that has already stopped talking. This kit ships the engine as a `Stop` hook (`hooks/stop-ledger`): while any row is `todo` or `building` it refuses the exit and hands back the open rows as an error, with the continuation contract — *the goal persists across turns; do not redefine success around what already works; take ONE row to a real close.* Two properties matter more than the blocking. It **counts progress as rows resolved, never as turns or edits**, so after three stops with nothing resolved it stops nudging and escalates instead — a loop that generates activity instead of closures is the imitation of work, not the cure for it. And it **fails open everywhere**: no ledger, no `jq`, malformed JSON, or `COMPOUND_V_LEDGER_GATE=off` and it disappears rather than wedging a session. `bash scripts/ledger.sh --open` prints the same number for a human, and exits non-zero while anything is open — compute the fraction, never assert it.

If you are running somewhere this hook is not installed, say at the start of the run what *does* re-invoke you, because otherwise the gate has nobody to gate. Four rules hold it up — **references/completion-ledger.md** carries the row shape and the operational detail; read it when you open a ledger and again at the gate.

1. **Denominator.** Rows come from the PRD or brief, the plan, and whatever the build discovers. Every requirement maps to a row and every row to a slice — **a requirement with zero rows is scope nobody was assigned**, the largest single source of the missing 10%, because it never looked like it was failing; it never looked like anything.
2. **Flip.** A row goes green only when its check was seen **red first, for the right reason** (a check that passes against an empty implementation measures nothing — **compound-v:test-driven-development**'s red step, applied to the ledger) and an end-to-end run was driven the way a user would, with one line recording what you ran.
3. **Delta.** Appending is always legal; flipping is not. Discovered work enters at `todo` the moment it is found, and the gate reports the movement — `declared 41 · discovered +7 · dropped −3 · passed 45 · todo 0` — so a moving denominator is visible rather than silent.
4. **Blocked is not done.** Three failed attempts makes a row `blocked`, blocker named; it counts against completion and escalates (**compound-v:systematic-debugging** owns the cap). Give the run a legal way to say *this one did not work* and it stops needing an illegal one — an agent facing an unpassable row under a keep-going instruction has exactly one other move, which is to rationalise a flip.

**The agent may not edit what grades it.** The only legal write is a status flip and its evidence; any other diff to a row, its steps, or a check the ledger leans on voids the completion claim until a person looks. Anthropic's reward-tampering work found models trained on gameable environments generalize to *directly rewriting their own reward function*, and where a harness can express this as a tool schema rather than a rule — a status enum the model may set, an objective it cannot reach — that is the stronger form and at least one shipped harness does exactly that.
...
**Attended runs get three *kinds* of scheduled interrupt and no others**: the Carve+Recon approval (the slices, the rows, the DELETE list), one line per landed slice, and the verdict. **Unattended runs get zero** — per **compound-v:using-compound-v**, take the reversible default, write it down, keep going.
