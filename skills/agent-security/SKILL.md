...
**2. Break the lethal trifecta.** An agent is exploitable for data theft when **private data + untrusted content + an outbound channel** meet in one flow (Simon Willison's framing): the untrusted content carries the injection, the private data is the loot, the channel is the exit. You rarely need all three at once — remove one leg for the flow that doesn't need it. No outbound channel on the path that reads attacker content; no private data in the context that fetches the web; no untrusted fetch in the agent that holds secrets. This is the constructive defense behind the vulnerability `recheck` only flags.
