...
- **The subject decides the judge.** Where a verification line's subject is *model-generated output*, it is an eval, not a test — route to **compound-v:evals** and name the judge and its alignment check, because no assertion asserts a distribution. Where the subject is a *user-facing surface*, add a **compound-v:product-taste** pass: "renders without error" is not "is not slop". Neither is mandatory, and reaching for both on a deterministic change with no model output and no UI is the padding this section exists to cut.
...
**The file is also what lets something other than you execute it.** A plan that lives in the conversation dies at the next compaction, cannot be diffed, and cannot be handed to a model that is not the one you are talking to; a plan at a path does all three, which is what makes the planner/executor split practical rather than theoretical — one model reasons the work into task files, a different vendor's model builds against them, and the artifact in between is a file either can read. Take that split where the execution is genuinely mechanical, and read `references/shapes.md`'s *Split the decider from the executor* row before you do: the handoff drops every decision the plan never stated, which is the same failure the global-constraints block exists to prevent and the reason that block is written for a reader who has only the file.
