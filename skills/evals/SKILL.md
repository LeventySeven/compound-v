...
**Write evaluators for failures you discover, not failures you imagine — don't do eval-driven development.** TDD's write-the-test-first instinct misfires here: an LLM's failure surface is effectively infinite, so evals authored up front mostly measure problems you guessed at while the real ones go uncaught. Error analysis comes first; each evaluator earns its place by pinning a failure you actually observed. (The one exception is a hard constraint you can state exactly up front — "never mention a competitor" — known before any trace.)

**Never let the model grade itself against its own prior output.** The gold label — the count, the attribution, the "what actually happened" — must anchor to an *externally resolved* authoritative source (the resolved outcome, a human verdict, the system of record), not to a number or summary the same model emitted earlier. A generate-and-grade-by-the-same-model loop confirms its own systematic error: it'll happily mark its hallucinated count "correct" because the hallucination is also the answer key. If the truth isn't externally pinned, you're measuring self-consistency, not correctness.
...
- **Pair every verdict with a one-line written critique** of *why* it failed. This is the highest-leverage trick in the whole skill: those critiques become the **few-shot examples for the LLM judge**, materially raising judge↔human agreement — in one documented loop, logging the response, the judge's critique, and the verdict and reviewing them with stakeholders lifted agreement from **68% to 94% over three iterations**. Critique first, score second.
...
- **Over-optimizing even a *legitimate* metric degrades the real thing (Goodhart).** This is the gaming you do to yourself: an eval can be perfectly fair and still hurt you if you push the system to ace it — chase factual-consistency and summaries go vague to stay safe (less wrong, also less relevant); chase needle-in-a-haystack recall and the model starts treating distractors as important. Don't ship a metric to its ceiling; watch whether climbing it still moves the outcome you actually care about, and stop when the two diverge.
