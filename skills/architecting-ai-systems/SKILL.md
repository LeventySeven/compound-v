...
- **Keep the harness model-forward and mostly self-written**, and **delete scaffolding on every model release** — the "product overhang" is when the capability already existed and only your scaffolding didn't (Boris Cherny, Anthropic, "Building Claude Code"). Prefer one universal interface (Bash) over dozens of bespoke tools.
