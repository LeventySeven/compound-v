...
**Where this hands next.** On a Large run the landing is not the end: **compound-v:get-shit-done**'s stage 4 is the product-level done-gate and it runs *after* this skill, once, over the assembled product — `using-compound-v` routes it that way and nothing else in the kit closes over the whole build. On Standard and below, landing is the end.
