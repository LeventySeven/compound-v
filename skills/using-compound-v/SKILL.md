...
| **Large** | multiple subsystems · a one-way door · schema or public API | split into sub-projects, each running its own Standard cycle under its own plan, with **one confirmed decomposition** above them on the PRD's core-functions spine — never a plan-of-plans. Nothing touches disk before a human approves it; fan out only across disjoint files, in worktrees. |
