...
**Resolving the ambiguity IS the task, not a precondition for it.** Real requests are dramatically shorter and vaguer than the tasks systems get benchmarked on — a median real prompt runs ~390 characters against 1,185–3,055 for benchmark tasks, while the median change it actually asks for is ~181 lines against 7–10. The gap between what the user typed and what they want is therefore the normal case, not a defect in the user: guessing intent well is part of the job, and guessing *silently* is the failure. Name the assumption you're proceeding under, in the frame, where it can be corrected cheaply. And interview rather than take dictation — the model is usually better at extracting requirements than the user is at stating them, so ask a few structured questions with concrete options and infer the rest from the codebase and surrounding context instead of demanding a complete spec up front.
