...
| 3 | `evals` → "a deliberately ugly baseline deck scored" | "a deliberately ugly baseline deck scored **2.8–4.0 out of 5**, and an image-quality judge awarded **5/5** to a deck that contained no image at all" | Two concrete experimental results carrying the whole "binary, not Likert" argument. |
...
| 7 | `context-engineering` → "≥80% is the bar, and major agent products run 90%+" | "Treat cache hit rate as a first-class metric — **≥80%** is the bar, and major agent products run **90%+**." | "Major agent products run 90%+" is an empirical claim about named-but-unnamed products. The ≥80% target alone would be a fine judgment call; the 90%+ observation is not. |
